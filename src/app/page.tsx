...
export default function HomePage() {
  ...
  const allSizes = ["SSS", "SS", "S", "M", "L", "XL", "2XL", "3XL", "4XL", "5XL", "6XL", "7XL", "8XL","9XL", "10XL"];
    ...
    const fetchStats = async () => {
      try {
        ...
        const apiProducts = data.stats.products.map((p: any) => ({
          ...
          imageUrl: p.imageUrl,
          type: "colored",
          sizes: p.variants // array ของ API
        }));
    ...
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-base-200 to-base-300" data-theme="light">
      ...
      <main className="flex-grow p-4 sm:px-6 lg:px-8 container mx-auto max-w-7xl">
        ...
        <div className="w-full p-4 rounded-xl mb-8">
          ...
          {loading && <p className="text-center text-lg">กำลังโหลดข้อมูลสินค้า...</p>}
          ...
          {!loading && !error && (
            <div className="relative w-full">
              ...
              <button
                onClick={scrollPrev}
                className="absolute left-2 top-1/2 -translate-y-1/2 btn btn-circle btn-secondary shadow-xl z-20"
              >
                ❮
              </button>
              ...
              <button
                onClick={scrollNext}
                className="absolute right-2 top-1/2 -translate-y-1/2 btn btn-circle btn-secondary shadow-xl z-20"
              >
                ❯
              </button>
              ...
              <div
                ref={carouselRef}
                className="carousel w-full rounded-box shadow-2xl border border-base-300 overflow-x-scroll snap-x snap-mandatory"
              >
                  ...
                  <div key={product.id} className="carousel-item relative w-full justify-center p-4 snap-center min-w-full">
                    <div className="card w-full lg:card-side bg-base-100 shadow-2xl hover:shadow-secondary/30 transition-all duration-500 ease-in-out transform hover:scale-[1.01]">
                      ...
                      <div className="card-body lg:w-1/2 justify-center text-center lg:text-left p-8">
                        ...
                        {/* SizeCard ปรับให้ครบทุกไซส์และเรียงตามลำดับ */}
                        <SizeCard
                          productType="colored"
                          sizes={allSizes.map((size) => {
                            const found = product.sizes.find((s: any) => s.size.toUpperCase() === size);
